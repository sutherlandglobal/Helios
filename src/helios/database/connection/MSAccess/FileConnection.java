package helios.database.connection.MSAccess;

import helios.database.connection.DatabaseConnection;
import helios.exceptions.DatabaseConnectionCreationException;
import helios.exceptions.InvalidSyntaxException;
import helios.util.IOLocker;
import helios.util.threading.ThreadPool;
...
public class FileConnection extends DatabaseConnection
{
	...
	private static final long SLEEP_INT = 5000; 
	...
	public ArrayList<String> getSchemaInfo(String tableName)
	{
		...
		if(database != null)
		{
			try 
			{
				...
				schemaInfo = new ArrayList<String>(table.getColumnCount());
	 ...
	 * 
	 * @see helios.database.connection.DatabaseConnection#runQuery(java.lang.String)
	 */
	@Override
	 ...
	 * @see helios.database.connection.DatabaseConnection#runQuery(java.lang.String)
	 */
	public ArrayList<String[]> runQuery(String query, int limit) 
	{
		...
		try 
		{
			thisQuery = new Query(query, queryResults);
		...
		try 
		{
			...
			retval = new ArrayList<String[]>(queryOutput);
	 ...
	 *
	 * @see helios.database.connection.DatabaseConnection#runParallelQueries(java.util.ArrayList)
	 */
	public Map<String, ArrayList<String[]>> runParallelQueries(ArrayList<String> queries)
	{
		ThreadPool tp = new ThreadPool(queries.size());

		final List<FileConnection> workers = Collections.synchronizedList(new ArrayList<FileConnection>(queries.size()));
	 ...
	 * 
	 * @see helios.database.connection.DatabaseConnection#setupConnection()
	 */
	@Override
	 ...
	 *
	 * @see helios.database.connection.DatabaseConnection#close()
	 */
	public void close() 
	{
		try 
	...
	class Query
	{
		 ...
		 */
		public Query(String query, QueryResults queryResults ) throws InvalidSyntaxException
		{
		 ...
		 * @return	The table name corresponding to the field util.parameter.
		 */
