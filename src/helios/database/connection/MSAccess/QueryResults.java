...
import helios.exceptions.InvalidConditionException;
		...
		if(line.containsKey(left))
		{
			...
			try
			{
				...
				else if(operator.equals("=}") )
				{
					...
					if(
							d.convertMSAccessDateToGregorian(line.get(left)).after(DateParser.convertSQLDateToGregorian(right)) ||
							d.convertMSAccessDateToGregorian(line.get(left)).equals(DateParser.convertSQLDateToGregorian(right))
						)
					{
						retval = true;
					}
				...
				else if(operator.equals("{="))
				{
					DateParser d = new DateParser();
					if(
							d.convertMSAccessDateToGregorian(line.get(left)).before(DateParser.convertSQLDateToGregorian(right)) ||
							d.convertMSAccessDateToGregorian(line.get(left)).equals(DateParser.convertSQLDateToGregorian(right))
							)
					{
						retval = true;
